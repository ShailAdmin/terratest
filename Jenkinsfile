    ...
    tools {
      terraform 'terraform'
        ...
        stages {
         ...
         stage('plan') {
            steps {
                sh 'pwd;cd TerraformSTA/; terraform init'
                sh "sh 'pwd;cd TerraformSTA/; terraform plan -out tfplan"
                sh 'pwd;cd TerraformSTA/; terraform show -no-color tfplan > tfplan.txt'
                }
            }
        stage('Approval') {
            when {
